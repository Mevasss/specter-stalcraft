...
bool ManualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	pTargetBase = reinterpret_cast<BYTE*>(LI_FN(VirtualAllocEx)(hProc, nullptr, pOldOptHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
	if (!pTargetBase) {
	...
	DWORD oldp = 0;
	LI_FN(VirtualProtectEx)(hProc, pTargetBase, pOldOptHeader->SizeOfImage, PAGE_EXECUTE_READWRITE, &oldp);
	...
	//File header
	if (!LI_FN(WriteProcessMemory)(hProc, pTargetBase, pSrcData, 0x1000, nullptr)) { //only first 0x1000 bytes for the header
	...
	BYTE* emptyBuffer = (BYTE*)LI_FN(malloc)(1024 * 1024 * 20);
	if (emptyBuffer == nullptr) {
		ILog("Unable to allocate memory\n");
		return false;
	}
	LI_FN(memset)(emptyBuffer, 0, 1024 * 1024 * 20);

	//CLEAR PE HEAD
	if (ClearHeader) {
		if (!LI_FN(WriteProcessMemory)(hProc, pTargetBase, emptyBuffer, 0x1000, nullptr)) {
	...
	if (ClearNonNeededSections) {
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				if ((SEHExceptionSupport ? 0 : LI_FN(strcmp)((char*)pSectionHeader->Name, ".pdata") == 0) ||
					LI_FN(strcmp)((char*)pSectionHeader->Name, ".rsrc") == 0 ||
					LI_FN(strcmp)((char*)pSectionHeader->Name, ".reloc") == 0) {
					ILog("Processing %s removal\n", pSectionHeader->Name);
					if (!LI_FN(WriteProcessMemory)(hProc, pTargetBase + pSectionHeader->VirtualAddress, emptyBuffer, pSectionHeader->Misc.VirtualSize, nullptr)) {
						ILog("Can't clear section %s: 0x%x\n", pSectionHeader->Name, LI_FN(GetLastError)());
					}
				}
			}
		}
	...
	if (AdjustProtections) {
		...
		for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
			if (pSectionHeader->Misc.VirtualSize) {
				...
				if (LI_FN(VirtualProtectEx)(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSectionHeader->Misc.VirtualSize, newP, &old)) {
		...
		DWORD old = 0;
		LI_FN(VirtualProtectEx)(hProc, pTargetBase, IMAGE_FIRST_SECTION(pOldNtHeader)->VirtualAddress, PAGE_READONLY, &old);
	}

	if (!LI_FN(WriteProcessMemory)(hProc, pShellcode, emptyBuffer, 0x1000, nullptr)) {
		ILog("WARNING: Can't clear shellcode\n");
	}
	if (!LI_FN(VirtualFreeEx)(hProc, pShellcode, 0, MEM_RELEASE)) {
		ILog("WARNING: can't release shell code memory\n");
	}
	if (!LI_FN(VirtualFreeEx)(hProc, MappingDataAlloc, 0, MEM_RELEASE)) {
